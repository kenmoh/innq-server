...
@user_router.post(
    "/guest-users", response_model=GuestResponse, status_code=status.HTTP_201_CREATED
)
async def create_company_user(
    user: UserCreate, supabase: Client = Depends(supabas_client.get_client)
):
...
@user_router.post(
    "/staff-users", response_model=StaffResponse, status_code=status.HTTP_201_CREATED
)
async def create_staff_user(
    user: StaffCreate,
    supabase: Client = Depends(supabas_client.get_client),
    current_user=Depends(supabas_client.get_company_user),
):
    return user_services.create_staff_user(
        supabase=supabase, user=user, current_user=current_user
    )
...
async def company_staff_user(
    ...
    return user_services.add_payment_gateway(
        supabase=supabase, current_user=current_user, data=data
    )


@user_router.post(
    "/no-post", response_model=NoPostResponse, status_code=status.HTTP_201_CREATED
)
...
@user_router.post(
    "/outlets", response_model=OutletResponse, status_code=status.HTTP_201_CREATED
)
...
@user_router.post(
    "/qrcodes", response_model=QRCodeResponse, status_code=status.HTTP_201_CREATED
)
...
@user_router.post(
    "/users/{user_id}/permission-groups/{group_id}", status_code=status.HTTP_201_CREATED
)
